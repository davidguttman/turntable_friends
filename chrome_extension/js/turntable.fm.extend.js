...
TFMEX.performMigrations = function() {
    var manifestVersion = $('body').attr('tt-ext-manifest-version')
    //populated by contentscript
...
TFMEX.preferencesView = function(cancelEvent, saveEvent) {
    ...
    [
    "dd", ["input#showDJChanges", {
        type: "checkbox",
        "data-tfmex-pref": "showDJChanges",
        value: 1
    }]
    ],
    [
    ...
    [
    "dd", ["input#showListenerChanges", {
        type: "checkbox",
        "data-tfmex-pref": "showListenerChanges",
        value: 1
    }]
    ]
    ]
    ...
    [
    "dd", ["input#tt-ext-enable-scrobbling", {
        type: "checkbox",
        "data-tfmex-pref": "enableScrobbling",
        value: 1
    }]
    ]
    ]
...
TFMEX.roomUserView = function(user) {
    ...
    returnObj.push(["a", {
        href: "javascript:TFMEX.showUserProfile('" + user.userid + "')"
    },
    user.name]);
    if (user.fbid !== "undefined") {
        returnObj.push(["a", {
            href: 'http://facebook.com/profile.php?id=' + user.fbid,
            target: "_blank",
            class: 'tt-ext-aux-link'
        },
        "on Facebook"]);
    }
    returnObj.push(["a", {
        href: 'http://ttdashboard.com/user/f/' + user.fbid + '/',
        target: "_blank",
        class: 'tt-ext-aux-link'
    },
    "on TTDashboard"])
		...
		*/
        ...
        try {
            ...
            var customMenuItems = [
            {
                ...
                elementId: "tt-ext-room-users-menu-item"
            },
            {
                ...
                elementId: "tt-ext-settings-menu-item"
            }
            ]

            $.each(customMenuItems,
            function(i, menuItem) {
            ...
            $('#tt-ext-mpd')[0].addEventListener('tt-ext-process-similar-songs',
            function() {
            ...
            $('#tt-ext-suggestions-link').live('click',
            function() {
            ...
            $('.tt-ext-suggested-song .tt-ext-search-link').live('click',
            function(evt) {
            ...
            function enableDesktopNotifications() {
                if (window.webkitNotifications && window.webkitNotifications.checkPermission() != 0) {
                    TFMEX.$body.bind('click.enableDesktopNotify',
                    function() {
            ...
            getRoomInfo = function() {
                ...
                try {
                    ...
                    console.error("Exception occured in getRoomInfo", e.stack)
                }
            },
			...
			*/
            ...
            updatePrefs = function() {
                ...
                if (!lastFmToken || lastFmToken === 'null') {
                    TFMEX.prefs.enableScrobbling = false
                    //this happens if the user cancels the auth
                }
            ...
            savePrefs = function() {
                ...
                if (!oldEnableScrobblingValue && enableScrobbling) {
                    turntable.showAlert("In order to enable last.fm scrobbling, you will now be taken to last.fm to authorize Turntable Extended to scrobble tracks on your behalf.",
                    function() {
                ...
                TFMEX.prefs["enableScrobbling"] = enableScrobbling
                //gets into local storage
            ...
            desktopAlert = function(notificationObj) {
                ...
                if (window.webkitNotifications && window.webkitNotifications.checkPermission() == 0) {
                    ...
                    notificationObj.image ? notificationObj.image: "",
                    // icon url - can be relative
                    notificationObj.title ? notificationObj.title: "",
                    // notification title
            ...
            raiseNewSongEvent = function(songMetadata) {
                ...
                if (typeof(m.command) !== "undefined") {
                    ...
                    case "deregistered":
                        if (TFMEX.prefs.showListenerChanges) {
                            ...
                            desktopAlert({
                                title:
                                m.user[0].name + " just " + listenerChangeMap[m.command] + " the room.",
                                image: "",
                                body: "",
                                timeout: TFMEX.prefs.messageTimeout
                            });
                    ...
                    case "rem_dj":
                        if (TFMEX.prefs.showDJChanges) {
                            ...
                            desktopAlert({
                                title:
                                m.user[0].name + " " + djChangeMap[m.command] + " the decks.",
                                image: "",
                                body: "",
                                timeout: TFMEX.prefs.messageTimeout
                            });
                    ...
                    case "update_votes":
                        ...
                        try {
                            ...
                            console.error("Exception occurred during showVote", e.stack);
                        }
                    case "update_user":
                    case "new_moderator":
                    default:
            ...
            console.error("Exception during initialization:", e.stack);
        }

        try {
            attachListeners();
        } catch(e) {
            console.error("Exception during attachListeners", e.stack);
        }
    };
    $.when(getTurntableObjects()).then(whenTurntableObjectsReady);
});
...
DGSTUFF.update_songs = function(song) {
  DGSTUFF.songs[song['_id']] = song.metadata;
  DGSTUFF.send_to_server('song', song);
}
...
DGSTUFF.log_vote = function(currentVote) {
  ...
  DGSTUFF.votes = DGSTUFF.votes || {};
  DGSTUFF.votes[songname] = DGSTUFF.votes[songname] || {};
  DGSTUFF.votes[songname][vote_name] = vote_value;
}
